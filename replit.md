...
-   **User Management**: Owner and admin interface for viewing all company users, editing roles, and removing users. Three role types: Owner (full permissions), Admin (can manage users and has full access), and Member (standard access). Protection against demoting or removing the last owner. Employees view accessible via left sidebar menu in main view (Opportunities/Contacts/Employees tabs).
...
-   **Sidebar Navigation**: Left sidebar menu with three options (Opportunities, Contacts, Employees) allows switching between different views. The sidebar uses toggle buttons with active state highlighting to show the current view. Each view renders in the main content area to the right of the sidebar.
