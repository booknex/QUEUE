...
-   **User Editing (Super Admin)**: Super admins can edit any user in the system via the "All Users" view. Each user card displays an edit button (pencil icon) that opens the EditUserModal with two tabs: Profile (edit username, email, firstName, lastName, and isSuperAdmin status) and Password Reset (set new password with confirmation). The modal includes real-time form validation with Zod schemas, uniqueness checks for username/email, and secure scrypt password hashing. The form state resets via useEffect when switching between users to prevent stale data bugs. API endpoints PATCH /api/users/:id and POST /api/users/:id/password enforce super admin-only access.
