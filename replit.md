...
-   **Opportunity Management**: Create, track, and edit opportunities, linked to contacts. Opportunity cards are clickable to edit details and support deletion with confirmation. **User Assignment**: Opportunities can be assigned to users within the same company. The assignment dropdown shows all company members and displays the assigned user's name on opportunity cards. Server-side validation ensures users can only assign opportunities to members of the same company, preventing cross-company data leakage.
