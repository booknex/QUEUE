...
-   **Multi-User Authentication**: Custom username/password authentication using Passport.js Local Strategy with scrypt password hashing. New users automatically receive a default company on registration with owner role. Users can belong to multiple companies with role-based access (owner/member). Sessions stored in PostgreSQL with secure httpOnly cookies (sameSite: 'none' for iframe embedding support).
