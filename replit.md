...
-   **Super Admin Capability**: Designated super admins (users with `is_super_admin='true'` in database) can access and manage ALL companies in the system, even those they're not members of. Super admins bypass all membership checks and can add/edit/delete users across any company. This provides system-wide oversight and administration capabilities.
-   **System-Wide User Management**: Super admins have exclusive access to "All Users" view via the Dashboard header triple-dot dropdown menu. This view displays all users across all companies in the system (not company-specific). The feature is secured with role-based authorization - only super admins can access the `/api/all-users` endpoint, which excludes sensitive password hashes from the response. Regular users and non-super-admins receive 403 Forbidden when attempting to access this endpoint.
-   **User Editing & Deletion (Super Admin)**: Super admins can edit and delete any user in the system via the "All Users" view. User cards are clickable to open the EditUserModal, with an explicit edit button (pencil icon) for affordance. The modal includes two tabs (Profile and Password Reset) plus a delete button at the bottom. Profile tab allows editing username, email, firstName, lastName, and isSuperAdmin status. Password Reset tab enables setting new passwords with confirmation. Delete functionality includes a destructive confirmation dialog that prevents super admins from deleting themselves. The modal features real-time form validation with Zod schemas, uniqueness checks for username/email, and secure scrypt password hashing. Form state resets via useEffect when switching between users to prevent stale data bugs. API endpoints enforce super admin-only access: PATCH /api/users/:id, POST /api/users/:id/password, and DELETE /api/users/:id (with self-delete prevention returning 400 status). User deletion automatically cascades to remove all company memberships via ON DELETE CASCADE.
...
-   **Header Navigation**: Dashboard header features a company selector dropdown and a triple-dot menu providing access to "All Users" (super admin only), "Company Management" (for current company user management), and logout functionality.
